    ...
    public class GridObjectMover : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (movingObject)
            {
                ...
                {
                    OnObjectStoppedMoving?.Invoke(movingObject.GetComponent<BuildableGridObject>().GetOwnGridSystem(), movingObject); //Event
        ...
        private void LateUpdate()
        {
            ...
            if (movingObject != null)
            {
                ...
                {
                    Vector3 targetPosition = currentActiveSystem.GetMouseWorldSnappedPositionForMoving(movingObject.GetComponent<BuildableGridObject>().GetBuildableGridObjectTypeSO()); //Call 'GetMouseWorldSnappedPositionForMoving' in 'GridBuildingSystem3D' and assign the returned position in 'targetPosition'
                    ...
                    transform.rotation = Quaternion.Lerp(transform.rotation, currentActiveSystem.GetPlacedObjectRotationForMoving(movingObject.GetComponent<BuildableGridObject>().GetBuildableGridObjectTypeSO()), Time.deltaTime * 25f); //Smoothly lerp through the current rotation and target rotation grabbed from 'GetPlacedObjectRotation' fuction
                }
                else
                {
                    Vector3 targetPosition = currentActiveSystem.GetMouseWorldSnappedPositionForMoving(movingObject.GetComponent<BuildableGridObject>().GetBuildableGridObjectTypeSO()); //Call 'GetMouseWorldSnappedPositionForMoving' in 'GridBuildingSystem3D' and assign the returned position in 'targetPosition'
        ...
        public void TriggerBuildableMove()
        {
            ...
            if (isBuildableMoveActive && !ASTLibrary.IsPointerOverUI())
            {
                if (currentActiveSystem.GetGridMode() == GridMode.None || currentActiveSystem.GetGridMode() == GridMode.Moving)
                {
                    if (currentActiveSystem.gridAxis == GridAxis.XZ)
        ...
        private void HandleBuildingMovingXZ()
        {
            ...
            if (currentActiveSystem.GetGridObjectXZ(GetPlacedObjectCalculatedMouseWorldPositionXZ()) != null || currentActiveSystem.GetGridObjectXZ(GetMouseWorldPosition()) != null)
            {
                ...
                if (buildableGridObject != null)
                {
                    ...
                    OnObjectStartMoving?.Invoke(movingObject.GetComponent<BuildableGridObject>().GetOwnGridSystem(), movingObject); //Event
                    HookMovingObject();
                }
                else
                {
