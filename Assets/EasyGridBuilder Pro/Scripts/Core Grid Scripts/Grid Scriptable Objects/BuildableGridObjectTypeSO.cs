...
{
    [CreateAssetMenu(menuName = "SoulGames/Easy Grid Builder Pro/BuildableGridObjectTypeSO", order = 0)] //Scriptable object asset path
    public class BuildableGridObjectTypeSO : ScriptableObject
    {
        ...
        public static Dir GetNextDirRight(Dir dir) //This function handles how rotation works
        {
            ...
            {
                default:
                case Dir.Down:      return Dir.Left;    //Down goes to Left
                case Dir.Left:      return Dir.Up;      //Left goes to Up
                case Dir.Up:        return Dir.Right;   //Up goes to Right
                case Dir.Right:     return Dir.Down;    //Right goes back to Down
            }
        }

        public static Dir GetNextDirLeft(Dir dir) //This function handles how rotation works
        {
            switch (dir) //Swtich use 'dir'
            {
                default:
                case Dir.Down:      return Dir.Right;    //Down goes to Left
                case Dir.Left:      return Dir.Down;      //Left goes to Up
                case Dir.Up:        return Dir.Left;   //Up goes to Right
                case Dir.Right:     return Dir.Up;    //Right goes back to Down
            }
        }
        ...
        public int GetRotationAngle(Dir dir) //This function return rotation an angle (Used to rotate object)
        {
            switch (dir) //Swtich use 'dir'
            {
                ...
                case Dir.Left:  return 90;      //If 'Left' rotate 90
                case Dir.Up:    return 180;     //If 'Up' rotate 180
                case Dir.Right: return 270;     //If 'Right' rotate 270
            }
        }
        ...
        public Vector2Int CalculatePlacedObjectSize(float cellSize)
        {
            ...
            float newObjectScaleX = objectPrefab[0].GetComponent<BuildableGridObject>().GetObjectScale().x;
            float newObjectScaleZ = objectPrefab[0].GetComponent<BuildableGridObject>().GetObjectScale().y;
